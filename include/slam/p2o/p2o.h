...
static inline double robust_coeff(p2o_float_t squared_error,
                                  p2o_float_t delta) {
  if (squared_error < 0)
    return 0;
...
struct Con2D {
  ...
  Pose2D t;
  Mat3D info;
  void setCovarianceMatrix(const Mat3D &mat) { info = mat.inverse(); }
...
struct ErrorFunc2D {
  static Vec3D error_func(const Pose2D &pa, const Pose2D &pb,
                          const Pose2D &con) {
  ...
  static Vec3D calcError(const Pose2D &pa, const Pose2D &pb, const Pose2D &con,
                         Mat3D &Ja, Mat3D &Jb) {
    ...
    Ja << -cos(pa.th), -sin(pa.th), dxdt, sin(pa.th), -cos(pa.th), dydt, 0, 0,
        -1;
    Jb << cos(pa.th), sin(pa.th), 0, -sin(pa.th), cos(pa.th), 0, 0, 0, 1;
...
public:
  Optimizer2D()
      : verbose(false), lambda(0), stop_thre(1e-3),
        robust_delta(std::numeric_limits<double>::max()) {}
  ~Optimizer2D() {}
  ...
  Pose2DVec optimizePath(const Pose2DVec &in_graphnodes,
                         const Con2DVec &constraints, int max_steps,
                         int min_steps = 3);
  double optimizePathOneStep(Pose2DVec &out_nodes, const Pose2DVec &graphnodes,
                             const Con2DVec &constraints, double prev_res);
...
Pose2DVec Optimizer2D::optimizePath(const Pose2DVec &in_graphnodes,
                                    const Con2DVec &constraints, int max_steps,
                                    int min_steps) {
  ...
  for (int i = 1; i <= max_steps; i++) {
    ...
    p2o_float_t res =
        optimizePathOneStep(ret, graphnodes, constraints, prevres);
    auto t1 = std::chrono::high_resolution_clock::now();
    auto elapsed =
        std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0);
    if (verbose)
      std::cout << "step " << i << ": " << res
                << " time: " << elapsed.count() * 1e-6 << "s" << std::endl;
    graphnodes = ret;

    if (i > min_steps && prevres - res < stop_thre) {
      if (verbose)
        std::cout << "converged: " << prevres - res << " < " << stop_thre
                  << std::endl;
      break;
    }
    prevres = res;
...
double Optimizer2D::globalCost(const Pose2DVec &poses,
                               const Con2DVec &constraints) {
  double sum = 0;
  for (const Con2D &c : constraints) {
    Vec3D diff = ErrorFunc2D::error_func(poses[c.id1], poses[c.id2], c.t);
    Mat3D info =
        c.info * robust_coeff(diff.transpose() * c.info * diff, robust_delta);
    sum += diff.transpose() * info * diff;
  }
  return sum;
}

double Optimizer2D::optimizePathOneStep(Pose2DVec &out_nodes,
                                        const Pose2DVec &graphnodes,
                                        const Con2DVec &constraints,
                                        double prev_res) {
  static const int dim = 3;
  size_t indlist[] = {0, 1, 2};
  size_t numnodes = graphnodes.size();
  Eigen::Matrix<p2o_float_t, Eigen::Dynamic, 1> bf =
      Eigen::Matrix<p2o_float_t, Eigen::Dynamic, 1>::Zero(numnodes * dim);
  ...
  for (const Con2D &con : constraints) {
    ...
    Mat3D info =
        con.info * robust_coeff(r.transpose() * con.info * r, robust_delta);
    ...
    for (size_t k : indlist) {
      for (size_t m : indlist) {
        tripletList.push_back(
            p2o_triplet(ida * dim + k, ida * dim + m, trJaInfoJa(k, m)));
        tripletList.push_back(
            p2o_triplet(idb * dim + k, idb * dim + m, trJbInfoJb(k, m)));
        tripletList.push_back(
            p2o_triplet(ida * dim + k, idb * dim + m, trJaInfoJb(k, m)));
        tripletList.push_back(
            p2o_triplet(idb * dim + k, ida * dim + m, trJaInfoJb(m, k)));
      }
    }
  ...
  for (unsigned int i = 0; i < graphnodes.size(); i++) {
    int u_i = i * dim;
    Pose2D pos(graphnodes[i].x + x[u_i], graphnodes[i].y + x[u_i + 1],
               graphnodes[i].th + x[u_i + 2]);
    out_nodes.push_back(pos);
  }
  return globalCost(out_nodes, constraints);
}

bool Optimizer2D::loadFile(const char *g2ofile, Pose2DVec &nodes,
                           Con2DVec &constraints) {
  Pose2DVec ret;
  ...
  while (is) {
    ...
    if (tag == "VERTEX_SE2") {
      ...
      sstrm >> con.id1 >> con.id2 >> x >> y >> th >> c1 >> c2 >> c3 >> c4 >>
          c5 >> c6;
      con.t = Pose2D(x, y, th);
      con.info << c1, c2, c3, c2, c4, c5, c3, c5, c6;
...
struct RotVec {
  ...
  Eigen::Quaterniond toQuaternion() const {
    ...
    return Eigen::Quaterniond(cos(v / 2), sin(v / 2) * ax / v,
                              sin(v / 2) * ay / v, sin(v / 2) * az / v);
  }
  Mat3D toRotationMatrix() const { return toQuaternion().toRotationMatrix(); }
};
...
static inline Eigen::Matrix<p2o_float_t, 4, 3> dQuat_dRV(const RotVec &rv) {
  ...
  if (rv.norm() < 1e-6) {
    dqu << -rv.ax, -rv.ay, -rv.az, 2, 0, 0, 0, 2, 0, 0, 0, 2;
  ...
  dqu << -u1 * S / vd, -u2 * S / vd, -u3 * S / vd,
      S / v + u1 * u1 * C / (2 * v2) - u1 * u1 * S / v3,
      u1 * u2 * (C / (2 * v2) - S / v3), u1 * u3 * (C / (2 * v2) - S / v3),
      u1 * u2 * (C / (2 * v2) - S / v3),
      S / v + u2 * u2 * C / (2 * v2) - u2 * u2 * S / v3,
      u2 * u3 * (C / (2 * v2) - S / v3), u1 * u3 * (C / (2 * v2) - S / v3),
      u2 * u3 * (C / (2 * v2) - S / v3),
...
static inline void dR_dRV(const RotVec &rv, Mat3D &dux, Mat3D &duy,
                          Mat3D &duz) {
  Eigen::Quaterniond q = rv.toQuaternion();
  double qw = q.w(), qx = q.x(), qy = q.y(), qz = q.z();
  Mat3D dRdqw, dRdqx, dRdqy, dRdqz;
  dRdqw << qw, -qz, qy, qz, qw, -qx, -qy, qx, qw;
  dRdqw *= 2;
  dRdqx << qx, qy, qz, qy, -qx, -qw, qz, qw, -qx;
  dRdqx *= 2;
  dRdqy << -qy, qx, qw, qx, qy, qz, -qw, qz, -qy;
  dRdqy *= 2;
  dRdqz << -qz, -qw, qx, qw, -qz, qy, qx, qy, qz;
  dRdqz *= 2;
  Eigen::Matrix<p2o_float_t, 4, 3> dqdu = dQuat_dRV(rv);
  dux = dRdqw * dqdu(0, 0) + dRdqx * dqdu(1, 0) + dRdqy * dqdu(2, 0) +
        dRdqz * dqdu(3, 0);
  duy = dRdqw * dqdu(0, 1) + dRdqx * dqdu(1, 1) + dRdqy * dqdu(2, 1) +
        dRdqz * dqdu(3, 1);
  duz = dRdqw * dqdu(0, 2) + dRdqx * dqdu(1, 2) + dRdqy * dqdu(2, 2) +
        dRdqz * dqdu(3, 2);
}

static inline Eigen::Matrix<p2o_float_t, 3, 4>
dRV_dQuat(const Eigen::Quaterniond &q) {
  if (1 - q.w() * q.w() < 1e-7) {
    Eigen::Matrix<p2o_float_t, 3, 4> ret;
    ret << 0, 2, 0, 0, 0, 0, 2, 0, 0, 0, 0, 2;
    return ret;
  }
...
static inline Eigen::Matrix<p2o_float_t, 4, 4> QMat(Eigen::Quaterniond &q) {
  ...
  Q << qw, -qx, -qy, -qz, qx, qw, -qz, qy, qy, qz, qw, -qx, qz, -qy, qx, qw;
  return Q;
}
...
struct Pose3D {
  ...
  Pose3D(p2o_float_t in_x, p2o_float_t in_y, p2o_float_t in_z,
         const RotVec &in_rv)
      : x(in_x), y(in_y), z(in_z), rv(in_rv) {}
  Pose3D(p2o_float_t in_x, p2o_float_t in_y, p2o_float_t in_z, p2o_float_t ax,
         p2o_float_t ay, p2o_float_t az)
      : x(in_x), y(in_y), z(in_z), rv(ax, ay, az) {}
  Vec6D vec() const {
    Vec6D v;
  ...
  Pose3D ominus(const Pose3D &base) const {
    Vec3D t =
        base.rv.toRotationMatrix().transpose() * (this->pos() - base.pos());
...
struct Con3D {
  ...
  Pose3D t;
  Mat6D info;
  void setCovarianceMatrix(const Mat6D &mat) { info = mat.inverse(); }
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
};
...
struct ErrorFunc3D {
  static Vec6D error_func(const Pose3D &pa, const Pose3D &pb,
                          const Pose3D &con) {
  ...
  static Vec6D calcError(const Pose3D &pa, const Pose3D &pb, const Pose3D &con,
                         Mat6D &Ja, Mat6D &Jb) {
    ...
    Ja.block<3, 3>(3, 3) =
        -dRV_dQuat(qdiff) * QMat(qcinv) * QMatBar(qb) * dQuat_dRV(rva_inv);
    Jb.block<3, 3>(3, 3) =
        dRV_dQuat(qdiff) * QMat(qcinv) * QMat(qainv) * dQuat_dRV(pb.rv);
...
public:
  ...
  void setRobustThreshold(double val) { robust_delta = val; }
  void setVerbose(bool val) { verbose = val; }
  Pose3DVec optimizePath(const Pose3DVec &in_graphnodes,
                         const Con3DVec &constraints, int max_steps,
                         int min_steps = 3);
  double optimizePathOneStep(Pose3DVec &out_nodes, const Pose3DVec &graphnodes,
                             const Con3DVec &constraints, double prev_res);
...
Pose3DVec Optimizer3D::optimizePath(const Pose3DVec &in_graphnodes,
                                    const Con3DVec &constraints, int max_steps,
                                    int min_steps) {
...
double Optimizer3D::globalCost(const Pose3DVec &poses,
                               const Con3DVec &constraints) {
  double sum = 0;
...
double Optimizer3D::optimizePathOneStep(Pose3DVec &out_nodes,
                                        const Pose3DVec &graphnodes,
                                        const Con3DVec &constraints,
                                        double prev_res) {
  ...
  for (unsigned int i = 0; i < graphnodes.size(); i++) {
    ...
    out_nodes.push_back(
        Pose3D(p.x + x[u_i], p.y + x[u_i + 1], p.z + x[u_i + 2], rv));
  }
  return globalCost(out_nodes, constraints);
}

bool Optimizer3D::loadFile(const char *g2ofile, Pose3DVec &nodes,
                           Con3DVec &constraints) {
  std::ifstream is(g2ofile);
  if (!is)
    return false;
