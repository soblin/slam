...
public:
  ...
  void Initialize();
  double CalcICPCovariance(const Pose2D &pose,
                           const std::vector<const ScanPoint2D *> &curPoints,
                           const std::vector<const ScanPoint2D *> &refPoints,
                           Eigen::Matrix3d &cov);
  // double CalcICPCovariance(const Pose2D &pose,
  //                          std::vector<const ScanPoint2D *> &&curPoints,
  //                          std::vector<const ScanPoint2D *> &&refPoints,
  //                          Eigen::Matrix3d &cov);
  double CalcPDistance(const ScanPoint2D *curPoint, const ScanPoint2D *refPoint,
                       double tx, double ty, double th);
  void CalcMotionCovarianceSimple(const Pose2D &motion, double dT,
                                  Eigen::Matrix3d &cov);
  void CalcMotionCovariance(double th, double dx, double dy, double dth,
  ...
  double CalcEigen(const Eigen::Matrix3d &cov, double *vals, double *vec1,
                   double *vec2);

  static void AccumulateCovariance(const Pose2D &curPose,
                                   const Pose2D &prevPose,
                                   const Eigen::Matrix3d &precCov,
